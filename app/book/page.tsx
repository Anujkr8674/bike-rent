  ...
  if (!slug) {
    return (
      <div className="page-wrap py-16 text-center">
        ...
        <Link href="/bikes" className="mt-4 inline-block text-[#FF6B1A] font-semibold hover:underline">
          Browse fleet →
        </Link>
      </div>
    );
  ...
  return (
    <div className="mesh-bg min-h-screen py-10 md:py-16">
      <div className="page-wrap max-w-5xl">
        <SectionReveal>
          <p className="text-sm font-semibold uppercase tracking-widest text-[#FF6B1A]">Guest booking</p>
          <h1 className="section-title mt-2 text-white">Book {bike.name}</h1>
          <p className="section-subtitle text-zinc-400">No account required · Pay securely · Upload documents after payment</p>
        </SectionReveal>

        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_1.1fr]">
          <div className="rounded-[24px] border border-white/10 bg-[#0A0A0A]/80 shadow-[0_8px_30px_rgba(0,0,0,0.4)] backdrop-blur-md overflow-hidden">
            ...
            <div className="p-5 border-t border-white/10">
              ...
              <p className="text-sm text-zinc-400">{bike.brand} · {bike.category}</p>
              <p className="mt-2 text-2xl font-bold text-white">{formatCurrency(bike.pricePerDay)}/day</p>
              <p className="text-sm text-zinc-400">{formatCurrency(bike.pricePerHour)}/hour</p>
            </div>
          </div>

          <div className="rounded-[24px] border border-white/10 bg-[#0A0A0A]/80 shadow-[0_8px_30px_rgba(0,0,0,0.4)] backdrop-blur-md p-6 md:p-8">
            <GuestBookingForm
              bike={bike}
