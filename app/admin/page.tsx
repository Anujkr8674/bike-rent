...
export default function AdminPage() {
      ...
      <SectionReveal>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="font-display mt-2 text-3xl font-bold text-white md:text-4xl">Dashboard</h1>
          ...
          <div className="rounded-3xl border border-white/10 bg-[#111111] p-5 shadow-xl shadow-black/40">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold text-white">Contact inquiries</h2>
              </div>
            ...
            <div className="mt-4 space-y-3">
              {(summary?.recentContacts ?? []).length === 0 ? (
                <p className="rounded-2xl border border-dashed border-white/10 p-5 text-sm text-zinc-400">
                  No contacts yet. The contact form will feed this section once wired to the database.
                </p>
              ) : (
                summary?.recentContacts.map((contact) => (
                  <div key={contact.id} className="rounded-2xl border border-white/5 p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold text-white">{contact.fullName}</p>
                        <p className="text-sm text-zinc-400">{contact.email}</p>
                      </div>
                      <span className="rounded-full bg-[#111111]/10 px-2.5 py-1 text-xs font-semibold text-zinc-300">
                        {contact.status}
                      </span>
                    </div>
                    <p className="mt-2 text-sm text-zinc-400">{contact.subject}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <SectionReveal delay={0.05}>
          <div className="rounded-3xl border border-white/10 bg-[#111111] p-5 shadow-xl shadow-black/40">
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold text-white">Jump into admin tasks</h2>
            </div>
            <div className="mt-4 space-y-3">
              {quickActions.map((action) => {
                ...
                return (
                  <Link
                    key={action.href}
                    href={action.href}
                    className="flex items-center justify-between rounded-2xl border border-white/5 px-4 py-3 transition hover:border-[#FF653F]/30 hover:bg-[#FF653F]/5"
                  >
            ...
            {/* <div className="mt-6 rounded-2xl bg-[#111111]/5 p-4">
              <p className="text-sm font-medium text-zinc-400">Next step</p>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
          ...
          <div className="rounded-3xl border border-white/10 bg-[#111111] p-5 shadow-xl shadow-black/40">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold text-white">Recent admin actions</h2>
              </div>
        ...
        {/* <SectionReveal delay={0.05}>
          <div className="rounded-3xl border border-white/10 bg-[#111111] p-5 shadow-xl shadow-black/40">
            ...
            <h2 className="mt-1 text-xl font-bold text-white">Prepared for scale</h2>
            <ul className="mt-4 space-y-3 text-sm text-zinc-400">
              <li className="rounded-2xl bg-[#111111]/5 px-4 py-3">Prisma-backed admin data model</li>
              <li className="rounded-2xl bg-[#111111]/5 px-4 py-3">Supabase Storage ready for bike media</li>
              <li className="rounded-2xl bg-[#111111]/5 px-4 py-3">Route-guarded admin authentication</li>
              <li className="rounded-2xl bg-[#111111]/5 px-4 py-3">CMS placeholders for content, contacts, and logs</li>
            </ul>
          </div>
        </SectionReveal> */}
        <SectionReveal delay={0.05}>
  <div className="rounded-3xl border border-white/10 bg-[#111111] p-5 shadow-xl shadow-black/40">
    ...
    <ul className="mt-4 space-y-3 text-sm text-zinc-400">
      <li className="rounded-2xl bg-[#111111]/5 px-4 py-3">
