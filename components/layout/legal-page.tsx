  ...
  return (
    <>
      ...
      <div className="page-wrap py-12 md:py-16">
        ...
        <div className="prose prose-zinc mt-10 max-w-3xl space-y-10">
          {sections.map((s, i) => (
            <SectionReveal key={s.heading} delay={i * 0.05}>
              <section>
                <h2 className="font-display text-xl font-bold text-zinc-900">{s.heading}</h2>
                {s.body.map((p, j) => (
                  <p key={j} className="mt-3 text-zinc-600 leading-relaxed">
                    {p}
                  </p>
                ))}
              </section>
